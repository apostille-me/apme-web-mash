...
mod tests {
    ...
    #[test]
    fn extracts_paths_from_fn_request_urls() {
        assert_eq!(
            request_path("https://example.test/api/heavy/render?x=1"),
            "/api/heavy/render",
        );
        assert_eq!(request_path("/api/heavy/render?x=1"), "/api/heavy/render");
        assert_eq!(request_path("api/heavy/render"), "/api/heavy/render");
    }

    #[test]
    fn unwraps_structured_cloud_events() {
    ...
    #[test]
    fn validates_fn_prefixed_binary_cloud_event_headers() {
        let headers = BTreeMap::from([
            ("fn-http-h-ce-specversion".to_owned(), "1.0".to_owned()),
            ("fn-http-h-ce-id".to_owned(), "event-1".to_owned()),
            ("fn-http-h-ce-source".to_owned(), "urn:ci".to_owned()),
            ("fn-http-h-ce-type".to_owned(), "example.created".to_owned()),
        ]);
        let payload = decode_payload(
            &headers,
            "application/json",
            br#"{\"document_id\":\"document-1\"}"#,
        )
        .unwrap();
        assert_eq!(payload["document_id"], "document-1");
    }
